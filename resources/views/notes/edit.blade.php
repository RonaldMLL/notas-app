...
@section('content')
    ...
    <form action="{{ route('notes.update', $note->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-blue-500" value="{{ $note->title }}">
        </div>

        <div>
            <label for="content" class="block text-gray-700 font-bold mb-2">Contenido:</label>
            <textarea name="content" id="content" rows="5" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-blue-500">{{ $note->content }}</textarea>
        </div>
